<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column :label="$t('table.actions')" align="center" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <a target="_blank" :href="row.ExportUrl">
            <el-button v-waves :loading="downloadLoading" class="filter-item" size="small" type="primary" icon="el-icon-download">
              {{ $t('table.export') }}
            </el-button>
          </a>
          ...
          <el-button v-if="row.status!='deleted'" size="small" icon="el-icon-delete" type="danger" @click="handleDelete(row)">
            {{ $t('table.delete') }}
          </el-button>
        </template>
      </el-table-column>
    ...
    <upload-json-component :on-success="handleSuccess" :before-upload="beforeUpload" />
...
<script>
import { fetchList, createRow, updateRow, deleteRow, uploadData } from '@/api/config_json'
import { pushConfig, mergeConfigInfo, mergeConfig } from '@/api/config'
import waves from '@/directive/waves' // waves directive
import Pagination from '@/components/Pagination' // secondary package based on el-pagination
import store from '@/store'
import TableForm from '@/components/TableForm'
import UploadJsonComponent from '@/components/UploadFile/json.vue'

export default {
  name: 'ComplexTable',
  components: { Pagination, TableForm, UploadJsonComponent },
  directives: { waves },
  filters: {},
  data() {
    return {
  ...
  methods: {
    render() {
      ...
      this.exportUrl = '/v1/json/' + this.listQuery.env_id + '/' + this.listQuery.collect + '/export'
      fetchList(this.listQuery).then(response => {
        this.list = response.data.items
        for (let i = 0; i < this.list.length; i++) {
          this.list[i].EditUrl = '/config/json?envId=' + this.listQuery.env_id + '&collect=' + this.$route.name + '&id=' + this.list[i].id
          this.list[i].ExportUrl = '/v1/json/' + this.listQuery.env_id + '/' + this.listQuery.collect + '/' + this.list[i].id + '/export'
        }
    ...
    handleSuccess(result) {
      this.listLoading = true
      uploadData(this.listQuery.env_id, this.listQuery.collect, result).then(() => {
        this.listLoading = false
        this.handleFilter()
        this.$notify({
          title: 'Success',
