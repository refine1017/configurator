<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="projectId" @change="handleChangeProject">
        <el-option v-for="item in projects" :key="item.id" :label="item.name" :value="item.id" />
      </el-select>

      <el-select v-model="listQuery.env_id" @change="handleFilter">
        <el-option v-for="item in environments" :key="item.id" :label="item.name" :value="item.id" />
      </el-select>

      <template v-for="(col ,index) in cols">
        <el-input
          v-if="checkFeature(col.feature, 'search')"
          v-model="listQuery[col.name]"
      ...
      <br>
      <a target="_blank" :href="exportUrl+'/json/all'">
        <el-button v-waves :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-download">
          {{ $t('table.exportJson') }}
        </el-button>
      </a>
      <a target="_blank" :href="exportUrl+'/lua/all'">
        <el-button v-waves :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-download">
          {{ $t('table.exportLua') }}
        </el-button>
      </a>
      ...
      <a target="_blank" :href="exportUrl+'/proto'">
        <el-button v-waves :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-download">
          {{ $t('table.exportProto') }}
        </el-button>
      </a>
      <a target="_blank" :href="exportUrl+'/entitas'">
        <el-button v-waves :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-download">
          {{ $t('table.exportEntitas') }}
        </el-button>
      </a>
      <a target="_blank" :href="exportUrl+'/cs'">
        <el-button v-waves :loading="downloadLoading" class="filter-item" type="primary" icon="el-icon-download">
          {{ $t('table.exportCS') }}
        </el-button>
      </a>
    ...
    >
      <template v-for="(col ,index) in cols">
        <el-table-column
          :prop="col.name"
          :sortable="checkFeature(col.feature, 'sort')"
          :label="col.name"
          align="center"
        >
    ...
    <el-dialog :title="textMap[dialogConfirmStatus]" :visible.sync="dialogConfirmVisible">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="dialogConfirmStatus==='push'?pushData():deleteData()">
          {{ $t('table.confirm') }}
        </el-button>
      </div>
    </el-dialog>
...
<script>
import { fetchList, createRow, updateRow, deleteRow, pushConfig } from '@/api/config'
...
export default {
  ...
  data() {
    return {
      ...
      environments: store.getters.userSelectEnvironments,
      ...
      exportUrl: '',
      listLoading: true,
      projectId: store.getters.userSelectProject.id,
      listQuery: {
        page: 1,
        limit: 20,
        env_id: store.getters.userSelectEnvironment.id
      },
      temp: {},
      tempId: -1,
  ...
  methods: {
    render() {
      ...
      this.exportUrl = '/v1/config/' + this.listQuery.env_id
      fetchList(this.listQuery).then(response => {
        this.list = response.data.items
        for (let i = 0; i < this.list.length; i++) {
          this.list[i].EditUrl = '/management/fields?envId=' + this.listQuery.env_id + '&index=' + this.list[i].index
        }
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          createRow(this.listQuery.env_id, this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateRow(this.listQuery.env_id, this.tempId, tempData).then(() => {
            this.dialogFormVisible = false
            this.render()
            this.$notify({
              title: 'Success',
    ...
    deleteData() {
      deleteRow(this.listQuery.env_id, this.deleteRow.index).then(() => {
    ...
    pushData() {
      this.dialogConfirmVisible = false
      pushConfig(this.listQuery.env_id, this.pushServer, 'all').then(response => {
        this.dialogConfirmVisible = false
